...
const EditJobForm = () => {
  ...
  const currentJob = useSelector((state) => selectJobById(state, jobId));
  const { baseSalary } = currentJob;
  const [formData, setFormData] = useState({
    jobTitle: currentJob.jobTitle || "",
    companyName: currentJob.companyName || "",
    jobType: currentJob.jobType || "",
    jobLocation: currentJob.jobLocation || "",
    jobDuration: currentJob.jobDuration || "",
    salarySpecified: currentJob.salarySpecified,
    currency: baseSalary && baseSalary.currency,
    amount: baseSalary && baseSalary.value && baseSalary.value.amount,
    frequency: baseSalary && baseSalary.value && baseSalary.value.frequency,
    applicationDate:
      moment(currentJob.applicationDate).utcOffset(0).format("YYYY-MM-DD") ||
      "",
    associatedEmail: currentJob.associatedEmail || "",
    applicationStatus: currentJob.applicationStatus || "",
    interviewDate:
      (currentJob.interviewDate &&
        moment(currentJob.interviewDate).format("YYYY-MM-DD HH:mm")) ||
      "",
    offerReceived: currentJob.offerReceived,
    offerDate:
      (currentJob.offerDate &&
        moment(currentJob.offerDate).utcOffset(0).format("YYYY-MM-DD")) ||
      "",
    offerReply: currentJob.offerReply,
    offerDecision: currentJob.offerDecision || "",
    attachedResumeName: currentJob.attachedResumeName || "",
    attachedCoverLetterName: currentJob.attachedCoverLetterName || "",
    followUp: currentJob.followUp || "",
    linkedinConnection: currentJob.linkedinConnection || "",
  ...
  return (
    <div className="form-box" style={{ width: "97%" }}>
      ...
      <form className="job-form edit" onSubmit={onSubmitHandler} noValidate>
        <FormInput
          label="Job Title"
          id="jobTitle"
          type="text"
        ...
        <Dropdown
          ...
          onChange={onChangeHandler}
        />
        {(salarySpecified === true || salarySpecified === "true") && (
          <>
            <FormInput
              label="Currency"
              id="currency"
        ...
        <Dropdown
          ...
          onChange={onChangeHandler}
        />
        {(offerReceived === true || offerReceived === "true") && (
          <>
            <FormInput
              label="Offer Date"
              id="offerDate"
            ...
            <Dropdown
              ...
              className="fade-in"
            />
            {(offerReply === true || offerReply === "true") && (
              <Dropdown
                label="Decision"
                id="offerDecision"
                name="offerDecision"
        ...
        <div className="btn">
          ...
          <Button buttonType="danger" type="button" onClick={onDeleteHandler}>
            Delete
          </Button>
        </div>
      </form>
